...
const AppMenuitem: React.FC<any> = (props) => {
    
    const location = useLocation();
    const menuContext = useContext(MenuContext);

    if (!menuContext) {
        throw new Error('AppMenuitem must be used within a MenuProvider');
    }
    ...
    return (
        <li className={classNames({ 'layout-root-menuitem': props.root, 'active-menuitem': active })}>
            ...
            {item.to && !item.items && item.visible !== false ? (
                <NavLink to={item.to} replace={item.replaceUrl} target={item.target} onClick={(e) => itemClick(e)} className={classNames(item.class, 'p-ripple', { 'active-route': isActiveRoute })} tabIndex={0}>
                    <i className={classNames('layout-menuitem-icon', item.icon)}></i>
                    <span className="layout-menuitem-text">{item.label}</span>
                    {item.items && <i className="pi pi-fw pi-angle-down layout-submenu-toggler"></i>}
                    <Ripple />
